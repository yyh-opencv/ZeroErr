...
static int ecx_main_send_processdata(ecx_contextt *context, uint8 group, boolean use_overlap_io)
{
   ...
   uint16 w1, w2;//地址分段
   int length;//数据长度
   uint16 sublength;//字段长度
   uint8 idx;//数据包索引
   int wkc;//工作技术器
   uint8* data;//数据指针
   boolean first=FALSE;//首次标志
   uint16 currentsegment = 0;//当前段
   uint32 iomapinputoffset;//IO映射偏移
   uint16 DCO;//分布时钟
...
int ecx_receive_processdata_group(ecx_contextt *context, uint8 group, int timeout)
{
   uint8 idx; //数据包索引
   int pos; //位置指针
   int wkc = 0, wkc2; //工作计数器
   uint16 le_wkc = 0;//小端工作计数器
   int valid_wkc = 0;//有效工作计数器
   int64 le_DCtime;//分布时钟时间
